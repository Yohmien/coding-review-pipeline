...
为真实工程而写的 Codex 技能——旨在保证编码工作流的可控性。它编排一条完整的变更流水线：探索 → 复杂计划追问 → 计划确认 → live 模型确认 → 任务契约 → coding 子代理实施 → 主会话独立复验 → fresh review → 完成验证。

流水线把架构、契约和风险决策留给主会话与用户，把实际 diff 和命令证据留给工作树；子代理报告只是 claims，不能替代证据。依赖被刻意做成**小、可组合、与模型无关**的独立 skill，按需加载，而不是把细则全部复制进本 skill。此外，用户可自行编排各个角色的模型选择及其推理强度，做到在保证低消耗的同时，完成高质量的代码。例如：可使用 GPT-5.6-sol-hight 进行决策、设计、复审，使用 GPT-5.6-luna-max 进行编码
...
4. **live 模型确认**——每次从 `spawn_agent` 的 live schema 读取模型与 `reasoning_effort`，推荐并请用户确认 coding/review 四项；任一项不受支持即停止报告，不静默替换或降级。按角色分层选模型：决策与设计、代码复审用强模型，coding 子代理用弱模型，尽可能降低 token 消耗。
